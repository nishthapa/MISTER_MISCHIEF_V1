...
robot_state = {
    ...
    "label_isImpactDetected": 0,   # <--- THE NEW CRASH LATCH 
...
def on_press(key):
    ...
    try:
        ...
        if hasattr(key, 'char') and key.char is not None:
            ...
            if c == 'u': set_orientation("label_isUpright")
            elif c == 'i': set_orientation("label_isUpsideDown") 
            
            elif c == 'w': robot_state["label_hazardDetected"] = 1
            elif c == 'c': robot_state["label_isImpactDetected"] = 1  # <--- PRESS
...
def on_release(key):
    ...
    try:
        if hasattr(key, 'char') and key.char is not None:
            ...
            elif c == 'c': robot_state["label_isImpactDetected"] = 0  # <--- RELEASE
